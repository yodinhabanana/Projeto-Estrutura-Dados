...
public:
    ...
    void mergeSort(athletes* vetor, int inicio, int fim);
...
// ordenacao
void Binario::particao(athletes* vetor, int inicio, int meio, int fim){

    int n1 = meio - inicio + 1;
    int n2 = fim - meio;

    athletes* esquerda = new athletes[n1];
    athletes* direita = new athletes[n2];
...
void Binario::mergeSort(athletes* vetor, int inicio, int fim){
...
void Binario::recursaoDividirArquivo(){
    ...
    // agora chama para juntar os arquivos com o heap lols
    juntarArquivos(i);
...
void Binario::dividirArquivo(ifstream& arquivoBinario, const string& nomeArquivo, int registrosPorBloco){
    ...
    // aloca dinamicamente
    athletes* reg = new athletes[registrosPorBloco];
    ...
    // ordenacao de cada bloco
    mergeSort(reg,0,registrosLidos-1);
    ...
    if (registrosLidos > 0){
        ...
        // reescrita dos blocos ordenados
...
//funcao pra juntar arquivos - os intercalando
void Binario::intercalaArquivos(){

    // reescrita no arquivo binario original
    ofstream ordenado(nomeArquivoBin, ios::binary);

    if(!ordenado){
        cout << "Erro na abertura do arquivo: " << nomeArquivoBin << "."  << endl;
        return;
    }

    cout << "em construcao bjsss" << endl;
...
void Binario::juntarArquivos(const int numeroArquivos){
